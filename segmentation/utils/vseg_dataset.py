...
class SegmentationDataset(Dataset):
    def __init__(
        ...
        self.image1_paths = df['image1'].tolist()
        self.seg_paths = df['segmentation'].tolist()
        assert len(self.image1_paths) == len(self.seg_paths)
    ...
    def __getitem__(self, idx):
        img_path = self.image1_paths[idx]
        mask_path = self.seg_paths[idx]

        img_np, img_spacing, _, _ = load_nifti(img_path)      
        mask_np, mask_spacing, _, _ = load_nifti(mask_path)   

        assert np.allclose(img_spacing, mask_spacing, atol=1e-2), f"Spacing mismatch: {img_path}"

        original_spacing = np.array(img_spacing)  

        center_voxel = get_center_of_label(mask_np, label=2)  # label=1 normal vessel pixels; label=2 occlusive pixel
        ...
        cropped_img = (cropped_img - self.window_center) / self.window_width  # [0, 1]
        ...
        normal_vessel_tensor = mask_tensor.clone()
        normal_vessel_tensor[normal_vessel_tensor==2] = 0
        input_tensor = torch.concat([image_tensor, normal_vessel_tensor], dim=0)

        weight_tensor = mask_tensor.clone()
        mask_tensor[mask_tensor > 0] = 1

        return input_tensor, mask_tensor, weight_tensor